...
This is a skeleton create-react-app project. The `src` folder has been divided into several subfolders to reflect the different code elements that make up a React app based on best practice.
...
It is extremely important to keep your API key private. For example, never include it in a project pushed up to GitHub. Notice, in `.gitignore` we included `.env` to safeguard against this.
